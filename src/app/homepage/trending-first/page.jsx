...
export default function TrendingFirstPage() {
  ...
  const fetchCountries = async () => {
    try {
      const res = await axios.get("/api/homepage/trending-first");
      if (res.data.ok) {
        setCountries(res.data.countries);
      }
    } catch {
      Swal.fire("Error", "Failed to fetch trending countries", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const saveCountry = async (country) => {
    ...
    try {
      const res = await axios.patch("/api/homepage/trending-first", country);
      if (res.data.ok) {
        Swal.fire({
          icon: "success",
          title: "Updated!",
          text: "Trending country updated successfully",
          timer: 1200,
  ...
  return (
    <div className="max-w-6xl mx-auto py-10">
      <h1 className="text-2xl font-bold mb-6">Edit Trending Countries (First Section)</h1>
